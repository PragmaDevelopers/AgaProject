...
export default function Page() {
    ...
    const [propostasState, setPropostasState] = useState<Proposta[]>([]);
    const [totalPriceVal, setTotalPriceVal] = useState<number>(0);
    const [hourPriceVal, setHourPriceVal] = useState<number>(0);
    const [projectTime, setProjectTime] = useState<number>(0);
    const [quantProf, setQuantProf] = useState<number>(0);
    const pageData00 = useRef<MDXEditorMethods>(null)
    const pageData01 = useRef<MDXEditorMethods>(null)
    const pageData02 = useRef<MDXEditorMethods>(null)
    const pageData03 = useRef<MDXEditorMethods>(null)
    const pageData04 = useRef<MDXEditorMethods>(null)
    const pageData05 = useRef<MDXEditorMethods>(null)
    const pageData06 = useRef<MDXEditorMethods>(null)

    useEffect(() => {
        fetch("http://localhost:8080/api/getall/propostas").then(response => response.json()).then(data => setPropostasState(data));
    }, [setPropostasState]);

    const validateCalcInput = (val: string): Boolean => {
        const regex: RegExp = /^(\d*\.?\d{0,2}|\d{1,3}(,\d{3})*(\.\d{0,2})?)$/;
        return regex.test(val);
    }

    const addNewProp = (event: any) => {
        event.preventDefault();
        const tempProps: Proposta[] = [
            ...propostasState,
            { Nome: event.target.title?.value, ID: generateRandomString(), Version: 0 }

        ];
        if (!validateCalcInput(event.target.hrtrab?.value)) return;

        setPropostasState(tempProps);
        console.log(propostasState);
        setShowCreate(false);
    }
    ...
    const hourOnChange = (e: any) => {
        ...
        setTotalPriceVal(((e.target.value * 240) * projectTime) * quantProf);
    }

    const projectTimeOnChange = (e: any) => {
        if (!validateCalcInput(e.target.value)) return;
        setProjectTime(e.target.value);
        setTotalPriceVal(((hourPriceVal * 240) * e.target.value) * quantProf);
    }

    const quantProfOnChange = (e: any) => {
        if (!validateCalcInput(e.target.value)) return;
        setQuantProf(e.target.value);
        setTotalPriceVal(((hourPriceVal * 240) * projectTime) * e.target.value);
    }

    const dadosPags: string[] = [
        `# Analytics - Reajuste de Preços NSPCLA2278\_2015
...
|     1.0    |  Versão Inicial | Marcelo Hideyuki | 24/05/2023 |`,
   ...
   5. * **Sprint Planning** é uma reunião na qual estão presentes o Product Owner, o Scrum Master e todo o Scrum Team, bem como qualquer pessoa interessada que esteja representando a gerência ou o cliente. Durante o Sprint Planning, o Product Owner descreve as funcionalidades de maior prioridade para a equipe. A equipe faz perguntas durante a reunião de modo que seja capaz de quebrar as funcionalidades em tarefas técnicas, após a reunião. Essas tarefas irão dar origem ao Sprint Backlog.
      ...
      * **Sprint Execution** é a execução diária das atividades que formam o Sprint. Ela não pode ser muito extensa e se restringir entre 2 a 4 semanas. Neste período as reuniões diárias Daily Scrum são realizadas a fim de solucionar possíveis impedimentos do Sprint.`,
        `1. Expertises e Diferenciais da Hitss
...
A **HITSS** não poderá ser responsabilizada por atrasos nos trabalhos decorrentes de fatores não controlados por ela, como falta de disponibilidade de recursos da **Claro**, atrasos nas instalações dos ambientes, atrasos em testes nos sistemas legados, indisponibilidade dos sistemas locais, legados, e de redes ou ainda indisponibilidade da documentação necessária para a realização dos trabalhos, dentre outros fatores.`,
    ...
    return (
        <main className="w-full h-full">
            <div className={(showCreate ? "flex" : "hidden") + " bg-black/25 w-screen h-screen justify-center items-center -mt-16 z-20"}>
                <div className="w-[80%] h-[80%] bg-neutral-50 relative rounded-md overflow-y-auto">
                    <button className="absolute top-1 right-2" onClick={() => setShowCreate(false)}>CLOSE</button>
                    <form onSubmit={addNewProp} className="flex flex-col justify-start items-start w-full h-full p-2">
                        <input type="text" placeholder="Nova Proposta" name="title" className="text-2xl bg-neutral-50 font-black" />
                        <div className="flex flex-row">
                            <input type="text" placeholder="Perfil Profissional" name="perfprof" />
                            <h1 className="mr-2">:</h1>
                            <input type="text" placeholder="Valor Hora/Mes" name="hrtrab" onChange={hourOnChange} />
                            <h1 className="mx-2">&times;</h1>
                            <input type="text" placeholder="Meses do projeto" name="projmon" onChange={projectTimeOnChange} />
                            <h1 className="mx-2">&times;</h1>
                            <input type="text" placeholder="Quantidade de profissionais" name="quantprof" onChange={quantProfOnChange} />
            ...
            <div className="flex flex-col">
                <main className='w-full h-full grid grid-cols-5 2xl:grid-cols-6 grid-flow-row auto-rows-auto gap-2 p-2'>
                    {propostasState.map((prop: Proposta, idx: number) => (
                        <div key={idx} className="p-2 border-2 border-neutral-400 rounded-md w-64 h-24">
                            <div className="flex flex-row justify-between items-center">
                                <h1>{prop.Nome}</h1>
                                <h2 className="border-2 border-neutral-300 rounded-full px-2 aspect-square text-sm flex justify-center items-center">{idx}</h2>
                            </div>
                        </div>
                    ))}
                </main>
                <button onClick={() => setShowCreate(true)} className="border-2 border-neutral-950 bg-neutral-50 hover:bg-neutral-950 hover:text-neutral-50 text-neutral-950 rounded-md p-2">
                    Nova Proposta
                </button>
            </div>
        </main>
